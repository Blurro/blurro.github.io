...
Birthday celebration page for Meluh — bouncing emoji balls, custom cursor, pastel background.

## Project overview

A single-page birthday animation built on TanStack Start (React SSR) + Tailwind CSS, deployed to Netlify.
...
src/
  routes/
    __root.tsx      — Root shell: loads Google Fonts (Pacifico + Nunito), sets page title
    index.tsx       — Entry route, renders BirthdayPage
  components/
    BirthdayPage.tsx — All animation logic: bouncing balls, cursor, confetti, sparkles
  styles.css        — Global CSS: background gradient, ball styles, cursor, text effects
```

## Architecture decisions

- **Pure client-side animation** — `requestAnimationFrame` loop drives all ball physics. No external animation lib.
- **DOM refs for ball positions** — ball elements are mutated directly via `style.transform` in the rAF loop (not React state) for 60fps without re-renders.
- **Cursor trail** — DOM nodes appended/removed on each `mousemove` throttled to 40ms; CSS animation handles fade-out.
- **Confetti** — Periodic burst via recursive `setTimeout`. Each piece is a raw DOM element with a CSS animation.
- **Ball-cursor deflection** — Distance check each frame; velocity replaced with normalised outward direction × strength when within collision radius.

## Customisation

- `PASTEL_COLORS` and `EMOJIS` arrays at top of `BirthdayPage.tsx` — easiest visual customisation.
- `NUM_BALLS` and `BALL_SIZE` — ball count and pixel size.
- `birthday-bg` CSS class / `bgShift` keyframes — background colour cycle speed and colours.

---

Original scaffold context below (may be outdated):
